...
console.log('========================================');
console.log('[Electron] main.cjs script loaded');
...
function createMainWindow() {
  ...
  if (!fs.existsSync(preloadPath)) {
    ...
    try {
      ...
      console.error('[Electron] Cannot read directory:', e);
    }
  }
...
// IPC handlers for updating floating windows
ipcMain.on('update-prompter', (event, data) => {
  console.log('[Electron] 📤 Received update-prompter IPC:', data);
  if (prompterWindow && !prompterWindow.isDestroyed()) {
    console.log('[Electron] 📤 Sending prompter-update to floating window');
    prompterWindow.webContents.send('prompter-update', data);
  } else {
    console.warn('[Electron] ⚠️ Prompter window not available, cannot send update');
  }
});

ipcMain.on('update-elf', (event, data) => {
  console.log('[Electron] 📤 Received update-elf IPC:', data);
  if (elfWindow && !elfWindow.isDestroyed()) {
    console.log('[Electron] 📤 Sending elf-update to floating window');
    elfWindow.webContents.send('elf-update', data);
  } else {
    console.warn('[Electron] ⚠️ Elf window not available, cannot send update');
  }
});
